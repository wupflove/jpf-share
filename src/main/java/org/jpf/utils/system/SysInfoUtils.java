...
import org.jpf.utils.JpfValidateUtil;
import org.jpf.utils.conf.JpfPropertiesUtil;
...
public class SysInfoUtils {
	...
	private static int km = 1024 * 1024;
	...
	public static void main(String[] args) {
		try {
			...
			System.err
					.println("最大可用内存(JVM):" + new DecimalFormat("#.#").format(maxMemorySize * 1.0 / 1024 / 1024) + "M");
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
	...
	public static long usedMemory() {
		...
		if (JpfValidateUtil.isValid(osmxb)) {
			return (osmxb.getTotalPhysicalMemorySize() - osmxb.getFreePhysicalMemorySize()) / km;
		}
		return 0;
	}
	...
	public static long JVMtotalMem() {
		return Runtime.getRuntime().totalMemory() / km;
	}
	...
	public static long JVMfreeMem() {
		return Runtime.getRuntime().freeMemory() / km;
	}
	...
	public static long JVMmaxMem() {
		return Runtime.getRuntime().maxMemory() / km;
	}
